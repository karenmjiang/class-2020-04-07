...
```{r}
x %>%
  summarize(correlation = cor(earnings, faculty))

x %>%
  summarize(correlation = cor(sat, earnings))

x %>%
  select(earnings, faculty, sat) %>%
  cor()
```
...
```{r}
model <- lm(earnings ~ sat + faculty, data = x)

model %>%
  tidy(conf.int = TRUE) %>%
  select(term, estimate, conf.low, conf.high)
```
Intercept estimate does not have a meaningful interpretation, since there is no value within the dataframe range where sat = 0 and faculty = 0. Additionally, a negative value for earnings does not make sense (unless they were paying money to their employer!)

The intercept is used to situatie the regression plane in 3D space.

Holding all else constant, for every point increase in SAT score, the student earnings increase by $24.48 on average 10 years out.

Holding all else constant, for every $ increase in Faculty earnings, the student earnings increase by $0.39 on average 10 years out.
...
Units: Institute of Higher Ed
Treatments: Decreasing the SAT score
Outcomes: Median earnings 10 years from now

The causal effect of of the policy change is what the earning will be for students.
The fundaental problem of causal inference is that we would not be able to observe the counterfactual.

We need to assume that there are no other confounders that would influence earnings (ie reputation, name recognition)
...
```{r}
predict(model, faculty = 50000, sat = 1200) %>%
  median()
```
